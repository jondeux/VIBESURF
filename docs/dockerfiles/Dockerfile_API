# VIBESURF: HuggingFace Spaces Production Dockerfile
...
# STARTUP SCRIPT: Create comprehensive startup script with error handling and port configuration
...
log "=== VibeSurf HuggingFace Spaces Startup ==="
...
for var in "${api_keys[@]}"; do
    if [ -n "${!var}" ]; then
        ...
        found_key=true
        # Export to environment for application use
        export "$var=${!var}"
    fi
done
...
log "Starting VibeSurf backend on $HOST:$PORT..."
